...
public:
  ...
  virtual bool on_joystick(float jx, float jy, float j2x, float j2y,
                           T &entity) {
    return false;
  }
...
public:
  ...
  void attach_render_system(Args &&...args) {
    render_systems_.emplace_back(
        std::make_unique<U>(std::forward<Args>(args)...));
  }
  template <typename U, typename... Args>
  void attach_physics_system(Args &&...args) {
    physics_systems_.emplace_back(
        std::make_unique<U>(std::forward<Args>(args)...));
  }
  template <typename U, typename... Args>
  void attach_controls_system(Args &&...args) {
    controls_systems_.emplace_back(
        std::make_unique<U>(std::forward<Args>(args)...));
  }
  ...
  void on_button(uint32_t buttons) override {
    for (auto it = controls_systems_.rbegin(); it != controls_systems_.rend();
         ++it)
